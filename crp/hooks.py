    ...
    def post_forward(self, module, input, output):
        ...
        def wrapper(grad):
            ...
            if isinstance(grad, tuple):
                result = tuple(hook_ref().backward(module, g) if g is not None else None for g in grad)
                return result
            else:
...
class FeatVisHook:
    ...
    def post_forward(self, module, input, output):
        ...
        activation = output_tensor.detach().to(self.on_device) if self.on_device else output_tensor.detach()
