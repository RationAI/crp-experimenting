...
class Maximization:
    ...
    def concatenate_with_results(
        self, layer_name, d_c_sorted, rel_c_sorted, rf_c_sorted
    ):
        ...
        if layer_name not in self.d_c_sorted:
            ...
            self.d_c_sorted[layer_name] = torch.cat(
                [d_c_sorted, self.d_c_sorted[layer_name]]
            )
            self.rel_c_sorted[layer_name] = torch.cat(
                [rel_c_sorted, self.rel_c_sorted[layer_name]]
            )
            self.rf_c_sorted[layer_name] = torch.cat(
                [rf_c_sorted, self.rf_c_sorted[layer_name]]
            )

    def sort_result_array(self, layer_name):

        d_c_args = torch.flip(
            torch.argsort(self.rel_c_sorted[layer_name], dim=0), dims=(0,)
        )
        if self.SAMPLE_SIZE is not None:
            d_c_args = d_c_args[: self.SAMPLE_SIZE, :]

        self.rel_c_sorted[layer_name] = torch.gather(
            self.rel_c_sorted[layer_name], 0, d_c_args
        )
        self.rf_c_sorted[layer_name] = torch.gather(
            self.rf_c_sorted[layer_name], 0, d_c_args
        )
        self.d_c_sorted[layer_name] = torch.gather(
            self.d_c_sorted[layer_name], 0, d_c_args
        )
    ...
    def _save_results(self, d_index: Tuple[int, int] = None):
        ...
        for layer_name in self.d_c_sorted:
            ...
            np.save(
                self.PATH / Path(filename + "data.npy"),
                self.d_c_sorted[layer_name].cpu().numpy(),
            )
            np.save(
                self.PATH / Path(filename + "rf.npy"),
                self.rf_c_sorted[layer_name].cpu().numpy(),
            )
            np.save(
                self.PATH / Path(filename + "rel.npy"),
                self.rel_c_sorted[layer_name].cpu().numpy(),
            )
